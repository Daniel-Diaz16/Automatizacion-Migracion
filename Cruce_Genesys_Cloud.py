...
# =============================================================================
# 2. FUNCIONES DE LIMPIEZA (ESTO REPARA EL ERROR)
# =============================================================================
...
reemplazos_nombres = {
    ...
    "Andrés Felipe Zizou Barrios Bermúdez": "Andres Felipe Zizou Barrios Bermudez",
    "AndrãS Felipe Zizou Barrios BermãDez": "Andres Felipe Zizou Barrios Bermudez",
...
if not archivos_csv:
    ...
    if lista_tablas:
        ...
        if "Usuarios" in base_final.columns:
            # 1. Extrae nombre entre guiones bajos y asigna "Discador" a vacíos
            base_final['Usuarios'] = base_final['Usuarios'].apply(
                crear_agente_inicial)
            # 2. Destruye dobles espacios y asegura mayúsculas iniciales
            base_final["Usuarios"] = base_final["Usuarios"].str.replace(
                r'\s+', ' ', regex=True).str.strip().str.title()
            # 3. Aplica diccionario de correcciones
            base_final["Usuarios"] = base_final["Usuarios"].replace(
                reemplazos_nombres)

        if "Fecha" in base_final.columns:
            # Verificamos si la tabla está vacía para no romper el código
            if base_final.empty:
                base_final['Fecha.'] = ""
                base_final['Hora'] = "00:00"
            else:
                # Aseguramos que sea texto para evitar bloqueos
                base_final['Fecha'] = base_final['Fecha'].astype(str)
                
                # Cortamos por el espacio en una tabla temporal
                partes = base_final['Fecha'].str.split(' ', n=1, expand=True)
                
                # 1. Verificamos si existe la columna 0 (Fecha)
                if 0 in partes.columns:
                    base_final['Fecha.'] = partes[0]
                else:
                    base_final['Fecha.'] = base_final['Fecha']
                
                # 2. Verificamos si existe la columna 1 (Hora)
        ...
        if "DNIS" in base_final.columns:
            base_final['Fono'] = base_final['DNIS'].apply(
                lambda x: str(
                    x)[-9:] if pd.notna(x) and str(x).strip() != "" else None
            )
        ...
        if "Tipificacion" in base_final.columns:
            ...
            base_final['Contacto'] = base_final['Tipificacion'].apply(
                lambda x: "Contacto" if pd.notna(
                    x) and x in lista_es_contacto else "No Contacto"
            )
            base_final['Contacto.'] = (
...
try:
    # CAMBIO PRINCIPAL: Usar read_excel y quitar sep/encoding
    ...
    base_final = base_final.rename(
        columns={'Segment_of_Origin': 'Bucket_1'})
    ...
    base_final = base_final.rename(
        columns={'Segment_of_Origin': 'Bucket_2'})
    ...
    base_final = base_final.rename(
        columns={'Segment_of_Origin': 'Bucket_3'})
...
try:
    # 1. Cargamos el CSV externo para la pestaña 'Acumulado'
    if os.path.exists(ruta_acomulado):
        # Leemos el archivo asegurando que los tipos de datos sean compatibles
    ...
    # 2. Escribimos el Excel con las 3 pestañas
