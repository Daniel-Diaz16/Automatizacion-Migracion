...
class TareaEjecutor:
    ...
    def _run(self):
        ...
        hora_inicio = datetime.now(ZoneInfo("America/Bogota"))
        ...
        try:
            ...
            if self.funcion and hasattr(modulo_importado, self.funcion):
                ...
                # Ejecutar el modulo directamente
                exec(open(f"{self.modulo}.py").read())
            
            self.ultimo_resultado = True
            self.ultimo_error = None
            hora_fin = datetime.now(ZoneInfo("America/Bogota"))
...
tarea_acumulado = TareaEjecutor("ACUMULADO", "Acomulado_Genesys_Cloud")
tarea_base_cloud = TareaEjecutor("BASE_CLOUD", "Base_Genesys_Cloud")
tarea_cruce = TareaEjecutor("CRUCE", "Cruce_Genesys_Cloud")
